...
public class PremierLeagueDAO {
	...
	public void listAllMatches(Map<Integer, Match> idMap) {
		...
		try {
			...
			ResultSet res = st.executeQuery();
			while (res.next()) {
				if (!idMap.containsKey(res.getString("m.MatchID"))) {
	...
	public List<Match> getMatchesOfMounth(Integer mese, Map<Integer, Match> idMap) {
		String sql = "SELECT DISTINCT * " + "FROM matches m " + "WHERE MONTH(m.Date) = ? ";
